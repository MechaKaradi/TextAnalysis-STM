...
setwd("C:\\Users\\Kaninik\\Programming\\Text_Analysis\\ProQuestPython\\NautilusPDFs\\splitfiles")
...
# Remove extra spaces
ports$text <- stringi::stri_replace_all_regex(ports$text, "\n", " ")
ports$text <- stringi::stri_replace_all_regex(ports$text, "\\s{2,}", " ")

# remove documents with less than 300 characters
ports <- subset(ports, nchar(ports$text) > 300)
